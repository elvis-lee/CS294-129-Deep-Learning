...
class HaoConvNet1_he(object):
  ...
  def __init__(self, input_dim=(3, 32, 32), num_filters=16, filter_size=5,
               hidden_dim=100, num_classes=10, reg=0.0,
               dtype=np.float32):
    ...
    weight_scale = input_c*filter_size**2
    weight_scale = (2./weight_scale)**0.5
    ...
    #output size (N, number_filters, H, W)
    #(H_out, W_out) = (H, W) because of the padding and stride config in the loss function, 
    weight_scale = num_filters*filter_size**2
    weight_scale = (2./weight_scale)**0.5
    ...
    weight_scale = num_filters*input_h*input_w/4.0
    weight_scale = (2./weight_scale)**0.5
