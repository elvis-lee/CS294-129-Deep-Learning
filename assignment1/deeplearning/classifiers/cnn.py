...
class ThreeLayerConvNet(object):
  ...
  def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
    ...
    #input size (N, F, H, W) 
    #output size (N, F, H/2, W/2)

    #hidden affine layer
    #input size (N, F, H/2, W/2)
    #output size (N, hidden_dim)
    self.params['W2'] = np.random.normal(scale = weight_scale, size = num_filters*(input_h/2)*(input_w/2)*hidden_dim).reshape((num_filters*(input_h/2)*(input_w/2),hidden_dim))
    self.params['b2'] = np.zeros(hidden_dim)
    
    #output affine layer
    #input size (N, hidden_dim)
    #output size (N, num_classes)
    self.params['W3'] = np.random.normal(scale = weight_scale, size = hidden_dim*num_classes).reshape((hidden_dim, num_classes))
