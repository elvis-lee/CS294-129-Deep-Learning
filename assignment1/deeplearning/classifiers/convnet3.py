...
class HaoConvNet3(object):
  """
  before:
  [conv-relu-pool] - [conv-relu-pool] - [conv-relu-pool] - [affine-bathnorm-relu] - dropout - affine - softmax
  now:
  [conv-spatialbathnorm-relu-pool] - [conv-spatialbathnorm-relu-pool] - [conv-spatialbathnorm-relu-pool] - [affine-bathnorm-relu] - dropout - affine - softmax
  """
  ...
  def __init__(self, input_dim=(3, 32, 32), num_filters=16, filter_size=5,
    ...
    self.bn_params = []
    self.bn_params = [{'mode': 'train'} for i in np.arange(4)]
  ...
  def loss(self, X, y=None):
    ...
    out0, cache0 = conv_spabachnorm_relu_pool_forward(X, W0, b0, conv_param, bn_param0, gamma0, beta0, pool_param)
    out1, cache1 = conv_spabachnorm_relu_pool_forward(out0, W1, b1, conv_param, bn_param1, gamma1, beta1, pool_param)
    out2, cache2 = conv_spabachnorm_relu_pool_forward(out1, W2, b2, conv_param, bn_param2, gamma2, beta2, pool_param)
    out3, cache3 = affine_batnorm_relu_forward(out2, W3, b3, gamma3, beta3, bn_param3)
    ...
    dx, grads['W3'], grads['b3'], grads['gamma3'], grads['beta3'] = affine_batnorm_relu_backward(dx, cache3)
    dx, grads['W2'], grads['b2'], grads['gamma2'], grads['beta2'] = conv_spabachnorm_relu_pool_backward(dx, cache2)
    dx, grads['W1'], grads['b1'], grads['gamma1'], grads['beta1'] = conv_spabachnorm_relu_pool_backward(dx, cache1)
    dx, grads['W0'], grads['b0'], grads['gamma0'], grads['beta0']= conv_spabachnorm_relu_pool_backward(dx, cache0)
