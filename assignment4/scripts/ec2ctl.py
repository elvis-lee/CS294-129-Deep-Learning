...
def get_all_instances():
    ...
    all_instances = [x for x in all_instances if get_tag_value(x, 'Owner') == config.attendee_id]
    ...
    for instance in get_all_instances():
        ...
        if exp_name == job:
            ...
            if 'latest.pkl' in pkl_files:
                ...
                pkl_file = sorted(pkl_files, key=lambda x: int(x.split('.')[0]))[-1]
            ...
            copy_command = [
                ...
                key_path,
                "ubuntu@{ip}".format(ip=ip_addr),
                "cp {remote_path} /tmp/params.pkl".format(remote_path=remote_pkl_path)
            ]
            ...
            local_pkl_path = os.path.join(local_exp_path, "snapshots", pkl_file)
...
def spot_history(instance_type, duration):
    ...
    if re.match(r"^(\d+)d$", duration):
        ...
        raise ValueError("Unrecognized duration: {duration}".format(duration=duration))

    with multiprocessing.Pool(100) as pool:
        print('Fetching the list of all availability zones...')
        zones = sum(pool.starmap(fetch_zones, [(x,) for x in config.aws_regions]), [])
