...
if __name__ == "__main__":
    ...
    for region in config.aws_regions:
        ...
        key_name = "{attendee_id}_{region}".format(attendee_id=config.attendee_id, region=region)
        ...
        try:
            ...
            if e.response['Error']['Code'] == 'InvalidKeyPair.Duplicate':
                if os.path.exists(file_name):
                    print("Key pair with name {key_name} already exists.".format(key_name=key_name))
                else:
                    print(
    ...
    print("aws_key_pairs:")
    for region in config.aws_regions:
        print("    - {region}: {key_name}".format(region=region, key_name=key_names[region]))
