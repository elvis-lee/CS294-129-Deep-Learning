...
def cartpole_test_grad_impl():
    ...
    test_theta = rng.uniform(size=(4, 5))
    # Check that the shape matches
    assert cartpole_get_grad_logp_action(test_theta, test_ob, test_act).shape == test_theta.shape
    gradient_check(
        lambda x: cartpole_get_logp_action(x.reshape(test_theta.shape), test_ob, test_act),
        lambda x: cartpole_get_grad_logp_action(x.reshape(test_theta.shape), test_ob, test_act).flatten(),
        test_theta.flatten()
    )
...
def main(env_id, batch_size, discount, learning_rate, n_itrs, render, natural, natural_step_size):
    # Check gradient implementation
    ...
    if env_id == 'CartPole-v0':
        ...
        raise ValueError("Unsupported environment: must be one of 'CartPole-v0', 'Point-v0'")
