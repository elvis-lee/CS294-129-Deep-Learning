...
register_test(
    ...
    kwargs=lambda: dict(
        old_dists=Gaussian(
            ...
            log_stds=Variable(nprs(1).uniform(size=(10, 3)).astype(np.float32)),
        ),
        new_dists=Gaussian(
            means=Variable(nprs(2).uniform(size=(10, 3)).astype(np.float32)),
            log_stds=Variable(nprs(3).uniform(size=(10, 3)).astype(np.float32)),
        ),
        all_acts=Variable(nprs(4).uniform(size=(10, 3)).astype(np.float32)),
        all_advs=Variable(nprs(5).uniform(size=(10,)).astype(np.float32)),
    ),
    desired_output=lambda: Variable(np.array(-0.5629823207855225, dtype=np.float32))
)
...
register_test(
    ...
    desired_output=lambda: Variable(np.array(0.5306503176689148, dtype=np.float32))
)
